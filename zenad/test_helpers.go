...
func init() {
	...
	// Note: Bech32 prefixes and coin type are already set by testutil/config/init.go
	// which is imported by this package. The SDK config is sealed after that,
	// so we don't need to set them again here.
}

func setup(withGenesis bool, invCheckPeriod uint, chainID string, evmChainID uint64) (*ZENAD, GenesisState) {
	db := dbm.NewMemDB()
...
// Setup initializes a new ZENAD. A Nop logger is set in ZENAD.
func Setup(t *testing.T, chainID string, evmChainID uint64) *ZENAD {
	t.Helper()
...
// SetupWithGenesisValSet initializes a new ZENAD with a validator set and genesis accounts
// that also act as delegators. For simplicity, each validator is bonded with a delegation
// of one consensus engine unit in the default token of the simapp from first genesis
// account. A Nop logger is set in ZENAD.
func SetupWithGenesisValSet(t *testing.T, chainID string, evmChainID uint64, valSet *cmttypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *ZENAD {
	t.Helper()
