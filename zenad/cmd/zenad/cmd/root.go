...
import (
	...
	evmdebug "github.com/zenanetwork/zena/client/debug"
...
func initRootCmd(rootCmd *cobra.Command, evmApp *zenad.ZENAD) {
	cfg := sdk.GetConfig()
	cfg.Seal()
...
func appExport(
	...
	if height != -1 {
		exampleApp = zenad.NewExampleApp(logger, db, traceStore, false, appOpts, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = zenad.NewExampleApp(logger, db, traceStore, true, appOpts, baseapp.SetChainID(chainID)) // TODO:VLAD - Remove // TODO:VLAD - Remove appoptions and evmchainid
	}
