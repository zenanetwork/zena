...
func initRootCmd(rootCmd *cobra.Command, evmApp *zenad.EVMD) {
	cfg := sdk.GetConfig()
	cfg.Seal()
	...
	baseappOptions = append(baseappOptions, func(app *baseapp.BaseApp) {
		...
		if maxTxs := cast.ToInt(appOpts.Get(server.FlagMempoolMaxTxs)); maxTxs >= 0 {
			...
			mpool = mempool.NewPriorityMempool(mempool.PriorityNonceMempoolConfig[int64]{
				...
				SignerExtractor: zenad.NewEthSignerExtractionAdapter(mempool.NewDefaultSignerExtractionAdapter()),
				MaxTx:           maxTxs,
			})
		} else {
...
func appExport(
	...
	if height != -1 {
		exampleApp = zenad.NewExampleApp(logger, db, traceStore, false, appOpts, evmdconfig.EVMChainID, evmdconfig.EvmAppOptions, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = zenad.NewExampleApp(logger, db, traceStore, true, appOpts, evmdconfig.EVMChainID, evmdconfig.EvmAppOptions, baseapp.SetChainID(chainID))
	}
