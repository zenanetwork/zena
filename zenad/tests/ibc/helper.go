...
func DeployContract(t *testing.T, chain *evmibctesting.TestChain, deploymentData testutiltypes.ContractDeploymentData) (common.Address, error) {
	...
	account := chain.App.(*zenad.EVMD).EVMKeeper.GetAccount(chain.GetContext(), from)
	if account == nil {
	...
	_, err = chain.App.(*zenad.EVMD).EVMKeeper.CallEVMWithData(chain.GetContext(), from, nil, data, true, nil)
	if err != nil {
