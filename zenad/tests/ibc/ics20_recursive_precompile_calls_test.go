...
func (suite *ICS20RecursivePrecompileCallsTestSuite) prepareStakingRewards(ctx sdk.Context, stkRs ...stakingRewards) (sdk.Context, error) {
	for _, r := range stkRs {
		...
		bondDenom, err := suite.chainA.App.(*zenad.ZENAD).StakingKeeper.BondDenom(suite.chainA.GetContext())
		...
		if err := suite.chainA.App.(*zenad.ZENAD).GetDistrKeeper().AllocateTokensToValidator(ctx, r.Validator, allocatedRewards); err != nil {
			return ctx, err
		}
	}
	return ctx, nil
...
func (suite *ICS20RecursivePrecompileCallsTestSuite) mintCoinsForDistrMod(ctx sdk.Context, amount sdk.Coins) error {
	...
	if err := suite.chainA.App.(*zenad.ZENAD).GetBankKeeper().MintCoins(
		ctx,
	...
	return suite.chainA.App.(*zenad.ZENAD).GetBankKeeper().SendCoinsFromModuleToModule(
		ctx,
		minttypes.ModuleName,
...
func (suite *ICS20RecursivePrecompileCallsTestSuite) setupContractForTesting(
	...
	evmAppA := suite.chainA.App.(*zenad.ZENAD)
