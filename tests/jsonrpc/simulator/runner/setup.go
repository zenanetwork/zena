...
import (
	...
	"github.com/zenanetwork/zena/tests/jsonrpc/simulator/contracts"
	"github.com/zenanetwork/zena/tests/jsonrpc/simulator/types"
	"github.com/zenanetwork/zena/tests/jsonrpc/simulator/utils"
)
...
func Setup() (*types.RPCContext, error) {
	...
	log.Printf("Created filter for ERC20 transfers: zenad=%s, geth=%s\n", rCtx.Evmd.FilterID, rCtx.Geth.FilterID)
...
// deployContracts deploys the ERC20 contract to both zenad and geth
...
// mintTokensOnBothNetworks distributes ERC20 tokens to specified accounts on both zenad and geth
func mintTokensOnBothNetworks(rCtx *types.RPCContext) error {
	...
	// Distribute on zenad (from dev0 who deployed the contract)
	fmt.Printf("Distributing tokens on zenad (contract: %s)...\n", rCtx.Evmd.ERC20Addr.Hex())
	evmdReceipts, err := distributeTokensOnNetwork(rCtx, distributionTargets, config.Dev0PrivateKey, false)
	if err != nil {
		return fmt.Errorf("failed to distribute tokens on zenad: %w", err)
	}
	...
	fmt.Printf("\nDistribution summary: zenad (%d/%d), geth (%d/%d)\n",
...
func newFilter(rCtx *types.RPCContext) error {
	...
	if err != nil {
		return fmt.Errorf("failed to create zenad filter: %w", err)
	}
