...
import (
	...
	"github.com/zenanetwork/zena/tests/jsonrpc/simulator/config"
)
...
// ComparisonResult holds the result of comparing API response structures between zenad and geth
...
func NewRPCContext(conf *config.Config) (*RPCContext, error) {
	// Connect to the primary Ethereum client (zenad)
...
func (rCtx *RPCContext) CompareRPCCallWithProvider(method string, paramProvider ParameterProvider) *ComparisonResult {
	...
	evmdParams := paramProvider(false) // false = zenad
	...
	if evmdErr == nil && gethErr == nil {
		...
		if evmdErr != nil && gethErr == nil {
			result.Differences = []string{"zenad request failed, geth succeeded - cannot compare structures"}
		} else if evmdErr == nil && gethErr != nil {
			result.Differences = []string{"geth request failed, zenad succeeded - cannot compare structures"}
		} else {
...
func (rCtx *RPCContext) findStructuralDifferences(evmdType, gethType string, evmdStruct, gethStruct map[string]string) []string {
	...
	if evmdType != gethType {
		differences = append(differences, fmt.Sprintf("Root type mismatch: zenad=%s, geth=%s", evmdType, gethType))
	}

	// Find missing fields in zenad
	for field, fieldType := range gethStruct {
		if _, exists := evmdStruct[field]; !exists {
			differences = append(differences, fmt.Sprintf("Missing field in zenad: %s (%s)", field, fieldType))
		}
	}

	// Find extra fields in zenad
	for field, fieldType := range evmdStruct {
		if _, exists := gethStruct[field]; !exists {
			differences = append(differences, fmt.Sprintf("Extra field in zenad: %s (%s)", field, fieldType))
		}
	}
	...
	for field, evmdFieldType := range evmdStruct {
		if gethFieldType, exists := gethStruct[field]; exists && evmdFieldType != gethFieldType {
			differences = append(differences, fmt.Sprintf("Type mismatch for %s: zenad=%s, geth=%s", field, evmdFieldType, gethFieldType))
		}
	}
...
// ParameterProvider is a function that provides parameters for zenad and geth separately
