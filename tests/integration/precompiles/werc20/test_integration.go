...
import (
	...
	"github.com/zenanetwork/zena/precompiles/werc20/testdata"
...
func TestPrecompileIntegrationTestSuite(t *testing.T, create network.CreateEvmApp, options ...network.ConfigOption) {
	_ = DescribeTableSubtree("a user interact with the WZENA precompiled contract", func(chainId testconstants.ChainID) {
		var (
		...
		Context("calling an erc20 method", func() {
			...
			When("querying information", func() {
				...
				It("should return the correct symbol", func() {
					...
					Expect(err).ToNot(HaveOccurred(), "failed to unpack result")
					Expect(symbol).To(ContainSubstring("ZNNT"), "expected different symbol")
				})
