...
function updateHardhatConfig(chainId, hardhatConfigPath) {
  try {
    ...
    // Try to find network block (zenanet or cosmos)
    const possibleNetworks = ['zenanet', 'cosmos'];
    let networkName = null;
    let networkBlockRegex = null;
    let match = null;

    for (const network of possibleNetworks) {
      const regex = new RegExp(`${network}:\\s*{([^{}]*(?:{[^{}]*}[^{}]*)*)}`);
      const testMatch = configContent.match(regex);
      if (testMatch) {
        networkName = network;
        networkBlockRegex = regex;
        match = testMatch;
        logger.info(`Found network block: ${network}`);
        break;
      }
    }

    if (match) {
      const networkBlock = match[1];
      const chainIdRegex = /chainId:\s*\d+/;

      if (chainIdRegex.test(networkBlock)) {
        const updatedNetworkBlock = networkBlock.replace(
          chainIdRegex,
          `chainId: ${chainId}`
        );
        const updatedContent = configContent.replace(
          networkBlockRegex,
          `${networkName}: {${updatedNetworkBlock}}`
        );

        fs.writeFileSync(hardhatConfigPath, updatedContent);
        logger.info(`Updated Hardhat config with chainId: ${chainId}`);
      } else {
        logger.warn(`chainId not found in ${networkName} network block`);
        logger.info("Network block content:", networkBlock);
      }
    } else {
      logger.warn("Could not find zenanet or cosmos network block in Hardhat config");
      logger.info(
        "Please check if your Hardhat config has a zenanet or cosmos network configuration"
      );
...
function checkTestEnv() {
  const argv = yargs(hideBin(process.argv))
    ...
    .example("$0 --network cosmos", "run all tests using zena network")
    .example(
    ...
    .boolean("verbose-log")
    .describe("verbose-log", "print zenad output, default false").argv;
...
function setupNetwork({ runConfig, timeout, chainId }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    ...
    const osdProc = spawn(scriptPath, ["-y"], {
      ...
      env: { ...process.env, CHAIN_ID: `zena_${chainId}-1` },
    });

    logger.info(`Starting zenad process... timeout: ${timeout}ms`);
    logger.info(`Chain ID: ${chainId}`);

    // Error handlers
    osdProc.on("error", (err) => {
      logger.err(`Failed to start zenad process: ${err.message}`);
      reject(err);
    });

    osdProc.on("exit", (code, signal) => {
      if (code !== 0 && code !== null) {
        logger.err(`zenad process exited with code ${code}, signal ${signal}`);
      }
    });

    // Force logging to debug (ignore verboseLog)
    osdProc.stdout.on("data", (d) => {
      const oLine = d.toString();
      console.log("[zenad stdout]:", oLine);
    ...
    osdProc.stderr.on("data", (d) => {
      const oLine = d.toString();
      console.error("[zenad stderr]:", oLine);
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error("Start zenad timeout!")), timeout);
  });
...
async function main() {
  ...
  // Extract chainId for use in setupNetwork
  const goConfigPath = path.join(__dirname, "../../server/config/config.go");
  const chainId = extractChainIDFromGo(goConfigPath);
  ...
  try {
    ...
    proc = await setupNetwork({ runConfig, timeout: 200000, chainId });
