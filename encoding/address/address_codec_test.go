...
func TestStringToBytes(t *testing.T) {
	...
	config.SetBech32PrefixForAccount("zenanet", "zenanetpub")
	...
	testCases := []struct {
		...
		},
		{
			...
			bech32,
			nil,
			sdkerrors.ErrLogic.Wrapf("hrp does not match bech32 prefix: expected '%s' got '%s'", "evmos", "zenanet"),
		},
		{
			"failure: invalid bech32 address (too long)",
			"zenanet",
			"zenanet10jmp6sgh4cc6zt3e8gw05wavvejgr5pwcyhngfv", // extra char at the end
			nil,
	...
	testCases := []tc{
		{
			...
			input: func() []byte {
				cdc = evmaddress.NewEvmCodec("zenanet")
				return addrBz
			},
			expRes: bech32,
			expErr: nil,
