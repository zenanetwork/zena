...
```diff
- github.com/zenanetwork/zena v0.3.1
+ github.com/zenanetwork/zena v0.4.0
```
...
Update your app’s `newApp` should return an **`evmserver.Application // from evmserver "github.com/zenanetwork/zena/server"`** rather than `servertypes.Application`, and CLI commands that still expect an SDK app creator require a wrapper.
...
+) evmserver.Application // from evmserver "github.com/zenanetwork/zena/server" {
   ...
 }
```
...
Add a new field for listeners (uses `ante.PendingTxListener // from "github.com/zenanetwork/zena/ante"`):
*(app/app.go)*

```diff
 type MyApp struct {
   ...
+  pendingTxListeners []ante.PendingTxListener // from "github.com/zenanetwork/zena/ante"
 }
```
...
```diff
- ibcTransferPrecompile, err := ics20precompile.NewPrecompile // from ics20precompile "github.com/zenanetwork/zena/precompiles/ics20"(
-     stakingKeeper,
+ ibcTransferPrecompile, err := ics20precompile.NewPrecompile // from ics20precompile "github.com/zenanetwork/zena/precompiles/ics20"(
+     bankKeeper,
+     stakingKeeper,
...
-     // from govprecompile "github.com/zenanetwork/zena/precompiles/gov"(govKeeper, cdc)
+ govPrecompile, err := govprecompile.NewPrecompile
+     // from govprecompile "github.com/zenanetwork/zena/precompiles/gov"(govKeeper, cdc, options.AddressCodec)
```
...
zenad export | jq '.app_state.erc20.dynamic_precompiles'
```
...
- [ ] `go.mod` has **no `replace`** lines for `github.com/zenanetwork/zena`.
