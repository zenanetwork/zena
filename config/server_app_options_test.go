...
func TestGetMinGasPrices(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupFn: func() servertypes.AppOptions {
				opts := newMockAppOptions()
				opts.Set(sdkserver.FlagMinGasPrices, "0.025uznnt")
				return opts
			},
			expected: sdk.DecCoins{sdk.NewDecCoinFromDec("uznnt", sdkmath.LegacyMustNewDecFromStr("0.025"))},
		},
		{
			name: "valid multiple gas prices",
			setupFn: func() servertypes.AppOptions {
				opts := newMockAppOptions()
				opts.Set(sdkserver.FlagMinGasPrices, "0.025uznnt,0.001stake")
				return opts
			},
			expected: sdk.DecCoins{
				sdk.NewDecCoinFromDec("stake", sdkmath.LegacyMustNewDecFromStr("0.001")),
				sdk.NewDecCoinFromDec("uznnt", sdkmath.LegacyMustNewDecFromStr("0.025")),
			},
		},
		{
		...
		},
		{
			...
			setupFn: func() servertypes.AppOptions {
				opts := newMockAppOptions()
				opts.Set(sdkserver.FlagMinGasPrices, "0uznnt")
				return opts
			},
			expected: sdk.DecCoins{},
		},
		{
			...
			setupFn: func() servertypes.AppOptions {
				opts := newMockAppOptions()
				opts.Set(sdkserver.FlagMinGasPrices, "0.000000000000000001uznnt")
				return opts
			},
			expected: sdk.DecCoins{sdk.NewDecCoinFromDec("uznnt", sdkmath.LegacyMustNewDecFromStr("0.000000000000000001"))},
		},
	}
