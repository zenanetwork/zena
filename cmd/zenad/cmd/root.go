...
import (
	...
	cosmosevmcmd "github.com/zenanetwork/zena/client"
	evmdconfig "github.com/zenanetwork/zena/cmd/zenad/config"
	cosmosevmkeyring "github.com/zenanetwork/zena/crypto/keyring"
...
func NewRootCmd() *cobra.Command {
	...
	initClientCtx := client.Context{}.
		...
		WithHomeDir(zenad.DefaultNodeHome).
	...
	if initClientCtx.ChainID != "" {
		if err := zenad.EvmAppOptions(cosmosevmserverconfig.DefaultEVMChainID); err != nil {
			panic(err)
		}
	}
...
func initRootCmd(rootCmd *cobra.Command, osApp *zenad.EVMD) {
	cfg := sdk.GetConfig()
	cfg.Seal()
	...
	rootCmd.AddCommand(
		genutilcli.InitCmd(
			...
			zenad.DefaultNodeHome,
		),
		genutilcli.Commands(osApp.TxConfig(), osApp.BasicModuleManager, zenad.DefaultNodeHome),
		cmtcli.NewCompletionCmd(rootCmd, true),
		debug.Cmd(),
		confixcmd.ConfigCommand(),
		pruning.Cmd(newApp, zenad.DefaultNodeHome),
		snapshot.Cmd(newApp),
	)
	...
	cosmosevmserver.AddCommands(
		rootCmd,
		cosmosevmserver.NewDefaultStartOptions(newApp, zenad.DefaultNodeHome),
		appExport,
		addModuleInitFlags,
	)
	...
	rootCmd.AddCommand(
		cosmosevmcmd.KeyCommands(zenad.DefaultNodeHome, true),
	)
...
func appExport(
	...
	if height != -1 {
		exampleApp = zenad.NewExampleApp(logger, db, traceStore, false, appOpts, evmdconfig.EVMChainID, zenad.EvmAppOptions, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = zenad.NewExampleApp(logger, db, traceStore, true, appOpts, evmdconfig.EVMChainID, zenad.EvmAppOptions, baseapp.SetChainID(chainID))
	}
