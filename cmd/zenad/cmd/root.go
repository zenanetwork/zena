...
func appExport(
	...
	if height != -1 {
		exampleApp = zenad.NewExampleApp(logger, db, traceStore, false, appOpts, zenad.EvmAppOptions, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = zenad.NewExampleApp(logger, db, traceStore, true, appOpts, zenad.EvmAppOptions, baseapp.SetChainID(chainID))
	}
