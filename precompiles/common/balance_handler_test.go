...
func setupBalanceHandlerTest(t *testing.T) {
	...
	// Bech32 prefix와 EVM 설정은 init()에서 자동 설정됨
	// 각 테스트마다 config를 리셋하여 초기 상태 유지
...
func TestParseHexAddress(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			maleate: func() sdk.Event {
				// 동적으로 올바른 체크섬을 가진 주소 생성
				hexAddr := "0x0000006B6579636861696e2D0000000000000001"
				addrBytes := common.HexToAddress(hexAddr).Bytes()
				bech32Addr, _ := sdk.Bech32ifyAddressBytes("zenanet", addrBytes)
				return sdk.NewEvent("bank", sdk.NewAttribute(banktypes.AttributeKeySpender, bech32Addr))
			},
			key:      banktypes.AttributeKeySpender,
