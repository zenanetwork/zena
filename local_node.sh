...
Options:
  ...
  --mnemonic-file PATH     Where to write mnemonics YAML (default: \$HOME/.zenad/mnemonics.yaml)
...
add_genesis_funds() {
  local keyname="$1"
  zenad genesis add-genesis-account "$keyname" 1000000000000000000000aznnt --keyring-backend "$KEYRING" --home "$CHAINDIR"
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  jq '.app_state["staking"]["params"]["bond_denom"]="aznnt"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="aznnt"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["params"]["min_deposit"][0]["denom"]="aznnt"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["params"]["expedited_min_deposit"][0]["denom"]="aznnt"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["evm"]["params"]["evm_denom"]="aznnt"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["mint"]["params"]["mint_denom"]="aznnt"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

  jq '.app_state["bank"]["denom_metadata"]=[{"description":"The native staking token for zenad.","denom_units":[{"denom":"aznnt","exponent":0,"aliases":["attoznnt"]},{"denom":"znnt","exponent":18,"aliases":[]}],"base":"aznnt","display":"znnt","name":"Zena Token","symbol":"ZNNT","uri":"","uri_hash":""}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  jq '.app_state.erc20.token_pairs=[{contract_owner:1,erc20_address:"0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",denom:"aznnt",enabled:true}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  zenad genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000aznnt --keyring-backend "$KEYRING" --home "$CHAINDIR"
  ...
  for ((i=0; i<${#dev_mnemonics[@]}; i++)); do
    ...
    echo "$mnemonic" | zenad keys add "$keyname" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
  ...
  if [[ "$ADDITIONAL_USERS" -gt 0 ]]; then
    ...
    for ((i=0; i<ADDITIONAL_USERS; i++)); do
      ...
      mnemonic_out="$(zenad keys add "$keyname" --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR" 2>&1)"
  ...
  # --------- Finalize genesis ---------
  zenad genesis gentx "$VAL_KEY" 1000000000000000000000aznnt --gas-prices ${BASEFEE}aznnt --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
