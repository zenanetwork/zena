...
# Start zenad node in background
start_node() {
	...
	if [ "$print_log" = true ]; then
		...
		./local_node.sh -y >/tmp/zenad.log 2>&1 &
	fi
	NODE_PID=$!
	popd >/dev/null
}
...
wait_for_node() {
	...
	local timeout=60
	local elapsed=0
	echo "Waiting for zenad node to be ready..."
	...
	if [ $elapsed -ge $timeout ]; then
		...
		tail -20 /tmp/zenad.log 2>/dev/null || echo "No zenad logs found"
		exit 1
	fi
}
