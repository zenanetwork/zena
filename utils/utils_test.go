...
import (
	...
	// Import testutil/config to trigger init() for Bech32 prefix setup
...
func TestGetAccAddressFromBech32(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"evmos address",
			"zenanet1qql8ag4cluz6r4dz28p3w00dnc9w8ueuh7ddla",
			"zenanet1qql8ag4cluz6r4dz28p3w00dnc9w8ueuh7ddla",
			false,
		},
		{
...
func TestAccAddressFromBech32(t *testing.T) {
	testCases := []struct {
		...
		{
			"zenanet1xv9tklw7d82sezh9haa573wufgy59vmw3vpra3",
			"stride",
			true,
			"invalid Bech32 prefix; expected stride, got zenanet",
		},
		{
...
func TestAddressConversion(t *testing.T) {
	// Bech32 prefix는 init()에서 자동 설정되고 sealed됨

	hex := "0x7cB61D4117AE31a12E393a1Cfa3BaC666481D02E"
	bech32 := "zenanet10jmp6sgh4cc6zt3e8gw05wavvejgr5pwcyhngf"
...
func TestHexAddressFromBech32String(t *testing.T) {
	// 테스트 주소를 동적으로 생성하여 올바른 체크섬 보장
	hexAddr := "0xd33bFF38Bfc79df581815BED0c779FA99BaFAbf5"
	addrBytes := common.HexToAddress(hexAddr).Bytes()

	accAddr, _ := sdk.Bech32ifyAddressBytes(constants.ExampleBech32Prefix, addrBytes)
	valAddr, _ := sdk.Bech32ifyAddressBytes(constants.ExampleBech32Prefix+"valoper", addrBytes)
	consAddr, _ := sdk.Bech32ifyAddressBytes(constants.ExampleBech32Prefix+"valcons", addrBytes)
